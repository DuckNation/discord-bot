...
class SMP(commands.Cog):
    ...
    async def setup_task(self):
        if not self.ready:
            ...
            chat_channels = await self.session.get(
                f"{self.bot.api_url}/chats/get?key={self.bot.api_key}&uuid_or_id=False"
            )
    ...
    async def create_channel(self, internal_id: str, name: str):
        ...
        async with self.session.get(
            f"{self.bot.api_url}/chats/get?key={self.bot.api_key}&chat_uuid={internal_id}"
        ) as resp:
    ...
    async def on_thread_member_join(self, thread_member: discord.ThreadMember):
        ...
        data = await self.session.get(
            f"{self.bot.api_url}/info/stats?key={self.bot.api_key}&uid={thread_member.id}"
        )
        if data.status != 200:
            await thread_member.thread.remove_user(discord.Object(thread_member.id))
            await thread_member.thread.send(
                embed=discord.Embed(
                    description="That member has not connected to the server!",
        ...
        if not data["uid"]:
            ...
            await thread_member.thread.send(
                embed=discord.Embed(
                    description="That member has not verified their discord!",
                    colour=discord.Colour.red(),
                )
            )
            return
        await thread_member.thread.send(
            embed=discord.Embed(
                description=f"Welcome <@{thread_member.id}>! ({data['username']})",
