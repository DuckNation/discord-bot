...
class DynamicTime(commands.Cog):
    ...
    async def cog_load(self) -> None:
        await self.sqlite.execute(
            "CREATE TABLE IF NOT EXISTS timezones (user_id INTEGER, timezone TEXT)"
        )
    ...
    async def setup(self, ctx: commands.Context):
        ...
        possible_vals = [
            ...
            if now.astimezone(pytz.timezone(tz)).hour == int(hour)
            and now.astimezone(pytz.timezone(tz)).minute == int(minute)
        ]
        ...
        await self.sqlite.execute(
            "DELETE FROM timezones WHERE user_id = ?", (ctx.author.id,)
        )
        await self.sqlite.commit()
        await self.sqlite.execute(
